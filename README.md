...
메일이 많을 때는 `/inbox` 로 먼저 거릅니다. 스팸, 물류·마케팅 영업, 소싱
에이전트를 빼고 **해외 매출로 이어질 수 있는 건만** A/B/C 등급으로 추려
표로 냅니다. C급은 건수와 유형만 세고, 사기 신호가 있는 건은 발신 주소와
함께 따로 알립니다. 등급이 나오면 볼 것만 `/email` 로 넘겨 번역과 답장을
받습니다.
